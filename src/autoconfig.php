...
class OutlookHandler extends RequestHandler {
    ...
    protected function parseRequest() {
        ...
        if (strlen($postdata) > 0) {
            ...
            return (object)array('email' => $xml->Request->EMailAddress,
                 'type' => $xml->Request->AcceptableResponseSchema);
        }
    ...
    if ($request->type == "http://schemas.microsoft.com/exchange/autodiscover/mobilesync/responseschema/2006") {
        $writer->writeAttribute("xmlns:autodiscover", "http://schemas.microsoft.com/exchange/autodiscover/mobilesync/responseschema/2006");
        $writer->startElement("autodiscover:Response");
        $writer->startElement("autodiscover:User");
        $writer->writeElement("autodiscover:EMailAddress", $request->email);
        $writer->endElement();
        $writer->startElement("autodiscover:Action");
        $writer->startElement("autodiscover:Settings");
        $writer->startElement("autodiscover:Server");
        $writer->writeElement("autodiscover:Type", "MobileSync");
        $writer->writeElement("autodiscover:Url", "SERVERURL");
        $writer->writeElement("autodiscover:Name", "SERVERURL");
        $writer->endElement();
